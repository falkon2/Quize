...
export default function SideBar() {
  ...
  return(
    <aside class="hidden sm:flex sm:flex-col bg-primary ">
    
    <div class="flex-grow flex flex-col justify-between text-gray-500 bg-primary  ">
            ...
            <select className="select select-primary w-full max-w-xs" data-choose-theme>
              <option className="text-primary" option value="">Default</option>
                {themeValues.map((value) => (
                  <option className='text-primary bg-secondary hover:bg-primary' key={value.toLowerCase()} value={value.toLowerCase()}>{value}</option>
                ) )}
            </select>
        </ul>
    </div>
