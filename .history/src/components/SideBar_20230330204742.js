  ...
  return(
    ...
    <div class="flex-grow flex flex-col justify-between text-gray-500   ">
      <nav class="flex flex-col mx-3 my-6 space-y-4">
        ...
        <div class="tooltip tooltip-primary tooltip-right flex py-3  items-center justify-center  h-20 w-20 border-t " data-tip="Settings">
        <div className="dropdown bg-primary dropdown-right dropdown-end">
        <label tabIndex={0} className="btn m-1"><IoSettings size={35} /></label>
          <ul tabIndex={0} className="dropdown-content menu p-2 shadow hover:text-secondary hover:bg-primary  rounded-box w-52">
            <select className="select select-primary w-full max-w-xs" data-choose-theme>
              <option className="text-secondary " option value="">Default</option>
                {themeValues.map((value) => (
                  <option className='text-secondary hover:bg-neutral' key={value.toLowerCase()} value={value.toLowerCase()}>{value}</option>
                ) )}
            </select>
        </ul>
    </div>
